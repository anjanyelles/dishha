...
import { isValidColorHex } from './string';
...
export function parseSbAssetUrl(assetUrl: string, options?: Partial<ImageAttributesOptions>) {
  ...
  if (options?.fill) {
    if (isValidColorHex(options.fill) || options.fill === 'transparent') {
      filters.fill = options.fill.replace(/^#/, '');
    }
  }
...
export function isCmsStory<B extends StoryblokStory<ISbComponentType>>(
  story: B | string
): story is B {
...
export function getStoryName(story: ISbStoryData) {
  return story.name;
}

const removeImageParagraphFromRichText = (document: ISbRichtext): ISbRichtext => {
  if (!document?.content) {
    return document;
  }
  ...
  return {
    ...
    content: document.content.map((block) => {
      if (block.type === 'paragraph' && block.content?.[0]?.type === 'image') {
...
export const cleanRichtext = (document: ISbRichtext): ISbRichtext => {
  return removeImageParagraphFromRichText(document);
};

export const typeLinkStory = (
  story:
    | string
    | StoryblokStory<PageStoryblok | ResourceStoryblok | CustomerStoryStoryblok | LegalStoryblok>
) =>
  story as StoryblokStory<
    PageStoryblok | ResourceStoryblok | CustomerStoryStoryblok | LegalStoryblok
  >;
