<script lang="ts" context="module">
  const isResourceStory = (story: ISbStoryData): story is ISbStoryData<ResourceStoryblok> => {
    return story.content.component === 'resource';
  };

  export let story: ResourceStoryblok;
</script>
...
{#if story && isResourceStory(story)}
  <article data-theme={story.content.theme || 'light'} class="overflow-clip bg-background">
    <slot />
    ...
    <!-- Highlight Card -->
    {#if $config?.highlight_card && $config?.highlight_card.length > 0}
      {#each $config?.highlight_card as b}
        <DynamicBlok blok={b} />
      {/each}
    {/if}
